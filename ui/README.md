...
│                     Admin UI (Port 5173)                 │
│  ┌──────────┐  ┌──────────────┐  ┌───────────────────┐  │
│  │  Share   │  │  Recipient   │  │   Audit Log       │  │
│  │ Manager  │  │   Manager    │  │    Viewer         │  │
│  └────┬─────┘  └──────┬───────┘  └────────┬──────────┘  │
│       │               │                   │             │
│  ┌────┴───────────────┴───────────────────┴──────────┐  │
│  │               API Service Layer                    │  │
│  │  shareApi / recipientApi / auditLogApi / configApi │  │
│  └───────────────────────┬───────────────────────────┘  │
└──────────────────────────┼──────────────────────────────┘
                           │  fetch()
              ┌────────────┴────────────┐
...
<App>
  <ThemeProvider>                  ← 主题状态管理
    <FluentProvider>               ← Fluent UI 主题注入
      <BrowserRouter>              ← 客户端路由
        <Navigation />             ← 顶部导航栏 (TabList)
        <Routes>
          /          → Navigate → /shares
          /shares    → <ErrorBoundary><ShareManager /></ErrorBoundary>
          /shares/:shareId/assets → <ErrorBoundary><ShareAssetDetail /></ErrorBoundary>
          /recipients → <ErrorBoundary><RecipientManager /></ErrorBoundary>
          /audit-logs → <ErrorBoundary><AuditLogViewer /></ErrorBoundary>
        </Routes>
      </BrowserRouter>
    </FluentProvider>
  </ThemeProvider>
...
|------|---------|---------|
| `/` | — | 自动重定向到 `/shares` |
| `/shares` | `ShareManager` | Share 实体列表，支持创建、重命名、删除 Share |
| `/shares/:shareId/assets` | `ShareAssetDetail` | 管理 Share 下的 Schema 和 Table 资产，支持添加/编辑/删除资产，以及从 DLC 同步表 |
| `/recipients` | `RecipientManager` | Recipient 列表，支持创建、编辑、删除 Recipient |
| `/audit-logs` | `AuditLogViewer` | 审计日志查看器，支持按类型、日期、自定义过滤条件查询 |
...
| 组件 | 文件 | 描述 |
|------|------|------|
| [Navigation](file:///c:/MOKURO/DEVELOP/Code/Project/delta_sharing_iceberg/ui/src/components/Navigation.tsx) | `Navigation.tsx` | 顶部导航栏，包含 Logo、标题、TabList（页面切换）和主题切换按钮 |
...
Component (useState + usePagination)
  → custom Hook (数据获取/分页逻辑)
    → API Service (fetch → /delta-sharing/admin/v1)
      → Vite Dev Proxy
        → Delta Sharing Server (FastAPI)
```
...
│   ├── components/
│   │   ├── index.ts                    # 组件统一导出
│   │   ├── Navigation.tsx              # 顶部导航栏（TabList + 主题切换）
